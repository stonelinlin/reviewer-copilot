...
from reviewer.entity_extract.providers import builtin_registry
...
def load_plugins_once() -> None:
  ...
  Discovers and loads entity_extract provider plugins using entry points.
  ...
  try:
    ...
    if hasattr(eps, "select"):
      # Python 3.10+ API
      provider_eps = eps.select(group="entity_extract.providers")
    elif hasattr(eps, "get"):
      # Python 3.9 API
      provider_eps = eps.get("entity_extract.providers", [])
    else:
      # Fallback for older versions
      provider_eps = [
          ep
          for ep in eps
          if getattr(ep, "group", None) == "entity_extract.providers"
      ]
...
def __getattr__(name: str):
  ...
  if name == "router":
    return importlib.import_module("entity_extract.providers.router")
  elif name == "schemas":
    return importlib.import_module("entity_extract.providers.schemas")
